...
local Button = Tab:CreateButton({
   ...
   Callback = function()
    if slockk then 
		game.StarterGui:SetCore("SendNotification", {Text="SLOCK: false", Title="SLOCK"})
		slockk = false
	else
		game.StarterGui:SetCore("SendNotification", {Text="SLOCK: true", Title="SLOCK"})
		slockk = true
	end
   end,
})
...
local Button = Tab:CreateButton({
   ...
   Callback = function()
    for i, destroyers in ipairs(workspace:GetDescendants()) do
wait(0.1)
notify("Destroyed", destroyers.Name, 5)
destroy(destroyers)
end
   end,
})
