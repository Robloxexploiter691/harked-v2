...
local script = G2L["6"];
	...
	script.Parent.MouseButton1Click:Connect(function()
		...
		getgenv().vulnremote = nil -- // if a remote with a vuln or backdoor is found it will be referenced in this variable
		...
		deletebind.Event:Connect(function(item)
			if backdoorfound == true then
				getgenv().vulnremote:FireServer(item)
			end
		end)
